...
func TestValidateReleaseDocsRejectsInvalidInputs(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "invalid tag", tag: "0.1.0", notes: strPtr(validReleaseNotes("v0.1.0")), changelog: changelogFor("v0.1.0"), wantErr: "tag must match"},
		{name: "missing release notes", tag: "v0.1.0", notes: nil, changelog: changelogFor("v0.1.0"), wantErr: "docs/release-notes/v0.1.0.md not found"},
		{name: "wrong title", tag: "v0.1.0", notes: strPtr(strings.Replace(validReleaseNotes("v0.1.0"), "# OpenBrief v0.1.0", "# OpenBrief 0.1.0", 1)), changelog: changelogFor("v0.1.0"), wantErr: `must start with "# OpenBrief v0.1.0"`},
		{name: "missing changed section", tag: "v0.1.0", notes: strPtr(strings.Replace(validReleaseNotes("v0.1.0"), "## Changed", "## Updates", 1)), changelog: changelogFor("v0.1.0"), wantErr: "must include ## Changed"},
		{name: "missing verification section", tag: "v0.1.0", notes: strPtr(strings.Replace(validReleaseNotes("v0.1.0"), "## Verification", "## Tests", 1)), changelog: changelogFor("v0.1.0"), wantErr: "must include ## Verification"},
		{name: "missing changelog link", tag: "v0.1.0", notes: strPtr(validReleaseNotes("v0.1.0")), changelog: "# Changelog\n\nNo matching release link.\n", wantErr: "CHANGELOG.md must link"},
		{
			name: "hard wrapped prose",
			tag:  "v0.1.0",
			notes: strPtr(`# OpenBrief v0.1.0
...
func validReleaseNotes(tag string) string {
	return "# OpenBrief " + tag + `

This release prepares the OpenBrief runner and skill for public distribution with documented release assets, verification, and agent eval evidence.

## Changed

- Added release automation, verification documentation, and production agent eval evidence.
- Hardened local quality gates, skill validation, and public repository metadata.
...
func changelogFor(tag string) string {
	return "# Changelog\n\n- [" + tag + "](https://github.com/yazanabuashour/openbrief/releases/tag/" + tag + ") prepares OpenBrief for public release.\n"
}
